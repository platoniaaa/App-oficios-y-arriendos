...
export function RolSwitcher({ user }: Props) {
  ...
  const items = [
    { id: 'cliente', label: 'Cliente', icon: UserRound, to: '/panel', active: path === '/panel' || (path.startsWith('/panel/') && !path.startsWith('/panel/prestador') && !path.startsWith('/panel/arrendador')), always: true },
