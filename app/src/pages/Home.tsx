...
import { ArrowRight, ShieldCheck, Sparkles, MessageSquare, Star, Wrench, Hammer, ClipboardList, Calendar } from 'lucide-react'
...
export function Home() {
  ...
  return (
    <div>
        ...
        <div className="container-page relative grid items-center gap-10 py-14 md:grid-cols-12 md:py-24">
          <div className="md:col-span-7 space-y-7 animate-fade-up">
            <div className="flex items-center gap-3 text-xs font-mono uppercase tracking-widest text-ink-500">
              ...
              {isProf ? 'Marketplace para contratistas · Chile' : 'Marketplace chileno · pagos protegidos'}
            </div>
            {isProf ? (
              <h1 className="font-display text-display-xl font-semibold tracking-tight text-balance">
                Tu obra completa,
                <br />
                <span className="italic text-ember">en un solo lugar</span>.
              </h1>
            ) : (
              <h1 className="font-display text-display-xl font-semibold tracking-tight text-balance">
                Encuentra al maestro <span className="italic text-ember">indicado</span>
                <br />
                para tu casa.
              </h1>
            )}
            <p className="max-w-xl text-lg text-ink-500 text-pretty">
              {isProf
                ? 'Subcontrata cuadrillas, arrienda equipos y coordina pagos protegidos. Cotiza tu obra entera con un solo proveedor — la IA arma el paquete completo.'
                : 'Te conectamos con maestros calificados de tu comuna. Pagas al final, cuando el trabajo esté listo. Si no sabes a quién llamar, el asistente IA te recomienda.'}
            </p>
            <div className="flex flex-wrap items-center gap-3">
              <Link to="/asistente" className="btn-ember btn-lg">
                <Sparkles className="h-4 w-4" />
                {isProf ? 'Cotizar mi obra con IA' : 'Arma tu proyecto con IA'}
              </Link>
              <Link to="/buscar/servicios" className="btn-outline btn-lg">
                {isProf ? 'Ver cuadrillas' : 'Ver oficios'} <ArrowRight className="h-4 w-4" />
              </Link>
            </div>
            <div className="flex flex-wrap items-center gap-5 pt-4 text-sm text-ink-500">
              <div className="flex items-center gap-2">
                <ShieldCheck className="h-5 w-5 text-moss" />
                {isProf ? 'Pagos y depósitos protegidos' : 'Escrow en cada trabajo'}
              </div>
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {isProf ? 'Comunicación centralizada' : 'Chat directo con el maestro'}
              </div>
            </div>
          </div>
            ...
            <div className="relative mx-auto max-w-sm">
              ...
              {isProf ? (
                ...
                <div className="ticket-sm rotate-[4deg] absolute -right-6 top-8 w-56 p-4 bg-navy text-cream">
                  ...
                  <p className="mt-2 font-display text-base leading-tight italic">
                    "Llegó puntual y dejó el baño impecable."
                  </p>
                  <p className="mt-3 text-xs text-cream/60">— Fernanda R., Ñuñoa</p>
                </div>
              )}
            </div>
          </div>
      ...
      <section className="bg-navy text-cream py-20">
        <div className="container-page">
          ...
          <h2 className="font-display text-4xl md:text-5xl font-semibold mt-1 mb-10 text-cream">
            {isProf ? (
              <>Orquesta tu obra, <br />sin perseguir proveedores.</>
            ) : (
              <>Contratar con confianza, <br />sin dramas en la cancha.</>
            )}
          </h2>
          <ol className="grid gap-6 md:grid-cols-4">
            {(isProf
              ? [
                  { n: '01', t: 'Cotiza con IA', d: 'Describe tu proyecto y te propone cuadrillas + equipos en minutos.' },
                  { n: '02', t: 'Aprueba el paquete', d: 'Filtra por especialidad, comuna y disponibilidad. Negocia desde un solo chat.' },
                  { n: '03', t: 'Ejecuta la obra', d: 'Calendario unificado, documentos, fotos de avance y comunicación centralizada.' },
                  { n: '04', t: 'Cierra y reseña', d: 'Liberas el pago al aprobar. Construyes historial con tus mejores cuadrillas.' },
                ]
              : [
                  { n: '01', t: 'Busca o pregúntale a la IA', d: 'Describe tu problema o elige directamente al maestro adecuado.' },
                  { n: '02', t: 'Acuerda y paga al escrow', d: 'Tu plata queda en custodia hasta que el trabajo esté listo.' },
                  { n: '03', t: 'Se ejecuta la pega', d: 'Chat directo con el maestro, fotos y visitas programadas.' },
      ...
      {/* TIPS / RECURSOS — solo particular */}
      {!isProf && (
        <section className="container-page py-16">
          <div className="mb-8">
            <p className="font-mono text-xs uppercase tracking-widest text-ember">04 — Para tu casa</p>
            <h2 className="font-display text-4xl font-semibold">Lo más pedido este mes</h2>
            <p className="mt-2 text-ink-500">Las pegas más comunes en hogares chilenos.</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              { titulo: 'Lavadora no enciende', icon: '🧺', cat: 'Técnico línea blanca' },
              { titulo: 'Aire acondicionado', icon: '❄️', cat: 'Climatización' },
          ...
          <div className="relative grid gap-8 md:grid-cols-2 md:items-center">
            <div>
              ...
              <h2 className="font-display text-4xl md:text-5xl font-semibold mt-6 leading-tight">
                {isProf ? (
                  <>Cotiza tu obra entera <br /><span className="text-ember italic">en 2 minutos.</span></>
                ) : (
                  <>Cuéntale tu problema al asistente. <br /><span className="text-ember italic">Te recomienda al maestro.</span></>
                )}
              </h2>
              <p className="mt-4 text-ink-500 max-w-md">
                {isProf
                  ? 'Describe tu proyecto y la IA arma cuadrillas + equipos + cotización inicial. Cierra con un solo proveedor.'
                  : 'La IA entiende qué necesitas, sugiere maestros de tu zona y te arma una cotización referencial en segundos.'}
              </p>
              <Link to="/asistente" className="btn-ember btn-lg mt-6 inline-flex">
                <Sparkles className="h-4 w-4" />
                {isProf ? 'Cotizar mi obra' : 'Probar asistente'}
              </Link>
            </div>
            <ul className="space-y-3">
              {(isProf
                ? [
                    '"Necesito cuadrilla y equipos para ampliación 80m²"',
                    '"Cotízame demolición + obra gruesa para 3 semanas"',
                    '"Quiero subcontratar instalación eléctrica + maestros"',
                    '"Arriendo de retro y andamio por 10 días"',
                  ]
                : [
                    '"Se me filtra el lavamanos del baño"',
                    '"Quiero pintar mi departamento de 60m²"',
                    '"Mi calefont no enciende desde ayer"',
                    '"Necesito mantener el jardín mensualmente"',
                  ]
              ).map((q) => (
                <li
                  key={q}
